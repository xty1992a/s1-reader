  ...
  <div class="page-forum">
    <div class="picked">
      <nut-cell-group title="已选">
        <nut-cell v-for="it in thread.forumList" :key="it.fid" :title="it.name">
          <template #link>
            <div class="set-block">
              <nut-icon name="arrow-up" @click="thread.stepForum(it, -1)"></nut-icon>
              <nut-icon name="arrow-down" @click="thread.stepForum(it, 1)"></nut-icon>
              <nut-icon name="close-little" @click="thread.delForum(it)"></nut-icon>
            </div>
          </template>
        </nut-cell>
      </nut-cell-group>
